...
{{-- Documents Table --}}
<div class="card animate-on-scroll" style="padding:0;overflow:hidden;transition-delay:0.4s;">
    <div style="padding:1.25rem 1.5rem;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;">
        ...
        <span class="badge" style="background:var(--primary-muted);color:var(--primary);">{{ $vendors->count() }} vendors</span>
    </div>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><span class="badge badge-{{ $vendor->status }}">{{ ucfirst($vendor->status) }}</span></td>
                    <td>
                        @if($vendor->id_card_path)
                            <a href="{{ Storage::url($vendor->id_card_path) }}" target="_blank" class="btn btn-outline btn-sm">
                                👁️ View KTP
                            </a>
                        @else
                            <span style="color:var(--text-muted);font-size:0.8rem;">— Not uploaded</span>
                        @endif
                    </td>
                  <td>
   @php
...
@endif
                    ...
                    <td style="color:var(--text-muted);font-size:0.8rem;">{{ $vendor->created_at->format('d M Y') }}</td>
                </tr>
                @empty
                <tr>
